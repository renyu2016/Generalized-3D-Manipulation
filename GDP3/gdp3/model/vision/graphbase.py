...
class GraphNetwork(nn.Module):
    def __init__(self, args):
        ...
        for l in range(self.num_layers):
            ...
            edge2node_net = NodeUpdateNetwork(
                ...
                num_features=self.node_features if l != self.num_layers - 1 else args.node_out_features,
                dropout=self.dropout if l < self.num_layers - 1 else 0.0)

            self.add_module('edge2node_net{}'.format(l), edge2node_net)
            self.add_module('node2edge_net{}'.format(l), node2edge_net)
        self.linear_proj = nn.Linear(3 * args.node_out_features, args.node_out_features)
    # forward
    def forward(self, init_node_feat, init_edge_feat, target_mask):
        ...
        node_features = node_feat_list[-1]
        node_feat_flat = node_features.reshape([node_features.shape[0], -1])
        
        
        graph_feat = self.linear_proj(node_feat_flat)
